...
type Action =
  ...
  | { type: "SET_CUSTOM_MESSAGE"; id: string; message: string }
  | { type: "CLEAR" };
...
const CartContext = createContext<{
  ...
  setCustomMessage: (id: string, message: string) => void;
  clear: () => void;
  subtotal: number;
  total: number;
} | null>(null);
...
function reducer(state: CartState, action: Action): CartState {
  switch (action.type) {
    case "ADD": {
      const qty = action.qty ?? 25; // Default to minimum quantity of 25
    ...
    case "SET_QTY":
      return {
        items: state.items
          .map((i) => (i.id === action.id ? { ...i, qty: Math.max(25, action.qty) } : i))
          .filter((i) => i.qty >= 25),
      };
    case "SET_CUSTOM_MESSAGE":
      return {
        items: state.items.map((i) =>
          i.id === action.id ? { ...i, customMessage: action.message } : i
        ),
      };
    case "CLEAR":
      return { items: [] };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const api = useMemo(
    () => ({
      ...
      setCustomMessage: (id: string, message: string) => dispatch({ type: "SET_CUSTOM_MESSAGE", id, message }),
