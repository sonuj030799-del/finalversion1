...
export default function Checkout() {
  const { state, subtotal, total, clear, setCustomMessage, remove, setQty } =
    useCart();
  const [customMessages, setCustomMessages] = useState<Record<string, string>>(
    state.items.reduce(
      (acc, item) => {
        acc[item.id] = item.customMessage || "";
        return acc;
      },
  ...
  const handleCustomMessageChange = (productId: string, value: string) => {
    setCustomMessages((prev) => ({ ...prev, [productId]: value }));
    setCustomMessage(productId, value);
  };
    ...
    return (
      <div className="container mx-auto py-12 px-4">
        ...
        <div className="mt-8 rounded-xl border border-dashed p-8 text-center text-muted-foreground max-w-md mx-auto">
          ...
          <Link
            to="/products"
            className="inline-block rounded-md bg-primary text-primary-foreground px-6 py-2 text-sm font-medium hover:bg-primary/90"
          >
            Continue Shopping
          </Link>
        </div>
  ...
  const orderText = state.items
    .map((i) => {
      let text = `${i.name} (Qty: ${i.qty})`;
      if (customMessages[i.id]) {
        text += `\nCustom Message: ${customMessages[i.id]}`;
      }
      return text;
    })
    .join("\n\n");
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              <tbody>
                {state.items.map((i) => (
                  ...
                  >
                    <td className="py-4">
                      <div className="flex flex-col gap-3">
                        ...
                        >
                          ...
                          />
                          <span className="font-medium text-sm md:text-base line-clamp-2 block text-primary hover:underline">
                            {i.name}
                          </span>
                        </Link>
                        <div>
                          ...
                          <textarea
                            ...
                            value={customMessages[i.id] || ""}
                            onChange={(e) =>
                              handleCustomMessageChange(i.id, e.target.value)
                            }
                            maxLength={1000}
                            rows={3}
                            className="w-full px-2 py-1.5 text-xs border rounded bg-white focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none"
                          />
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {(customMessages[i.id] || "").length}/1000
                          </p>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-end">
                        <div className="inline-flex items-center border border-gray-300 rounded">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition border-r border-gray-300"
                            title="Decrease quantity"
                          >
                            −
                          </button>
                          <span className="px-2 h-8 flex items-center justify-center font-medium text-sm">
                            {i.qty}
                          </span>
                          <button
                            onClick={() => setQty(i.id, i.qty + 1)}
                            className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition border-l border-gray-300"
                    ...
                    <td className="py-4 text-right font-semibold align-top">
                      {currency(i.price * i.qty)}
                    </td>
                    <td className="py-4 text-right align-top">
                      <button
                        onClick={() => remove(i.id)}
                        className="inline-flex items-center justify-center p-2 text-red-600 hover:bg-red-50 rounded transition border border-red-200 hover:border-red-300"
        ...
        <div className="h-fit rounded-xl border p-4 md:p-6 bg-accent/30 sticky top-4">
          ...
          <a
            href={(() => {
              const lines = state.items.map((i) => {
                let line = `${i.name} - Qty: ${i.qty} - Amount: ₹${(i.price * i.qty).toLocaleString("en-IN")}`;
                if (customMessages[i.id]) {
                  line += `\nCustom Message: ${customMessages[i.id]}`;
                }
                return line;
          ...
          <p className="mt-4 text-xs md:text-sm text-muted-foreground text-center">
            Use WhatsApp for instant confirmation or Email us and we'll follow
            up.
          </p>
